...
def _get_data(source, project_type):
    ...
    assert type(data[project_type.value]) == str or type(data[project_type.value]) == dict, 'Function not returning a String'

    return data[project_type.value]


def get_project_structure(project):
    assert isinstance(project, Project), 'Argument type not a Project.'
